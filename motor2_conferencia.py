...
PROD_DI, PROD_DF = ONTEM.isoformat(), HOJE.isoformat()
VEND_DI, VEND_DF = SETE_DIAS.isoformat(), HOJE.isoformat()
...
def upsert(tabela, registros, conflito):
    if not registros: return
    url=f"{SUPA_URL}/rest/v1/{tabela}?on_conflict={conflito}"
    ...
    for i in range(0,len(registros),500):
        ...
        for tent in range(3):
            try:
                with urllib.request.urlopen(req,timeout=180): break
...
print(f"MOTOR 2 - conferencia ({HOJE})")
print(f"  produtos: {PROD_DI} a {PROD_DF}")
print(f"  vendas:   {VEND_DI} a {VEND_DF}")
print("="*56)

print("\n[2a] Sincronizando produtos do dia anterior...")
produtos={}; ts=0; vistos=set(); loops=0
while True:
    loops+=1
    if loops>100: break
    body=f'<?xml version="1.0" encoding="utf-8"?><LinxMicrovix><Authentication user="linx_export" password="linx_export"/><ResponseFormat>xml</ResponseFormat><Command><Name>LinxProdutos</Name><Parameters><Parameter id="chave">{CHAVE}</Parameter><Parameter id="cnpjEmp">{CNPJ_REF}</Parameter><Parameter id="dt_update_inicio">{PROD_DI}</Parameter><Parameter id="dt_update_fim">{PROD_DF}</Parameter><Parameter id="timestamp">{ts}</Parameter></Parameters></Command></LinxMicrovix>'
    req=urllib.request.Request(LINX,data=body.encode("utf-8"),headers={"Content-Type":"text/xml; charset=utf-8"})
    try:
        with urllib.request.urlopen(req,timeout=180) as r: xml=r.read().decode("utf-8")
    except Exception as e: print("  erro, repetindo:",e); time.sleep(3); continue
    ...
    for r in rows:
        ...
        cp=g("cod_produto")
        if not cp or cp in vistos: continue
        vistos.add(cp); novo+=1
        produtos[cp]={"cod_produto":inteiro(cp),"nome":g("nome") or None,
          "id_setor":inteiro(g("id_setor")),"desc_setor":g("desc_setor") or None,
          "id_linha":inteiro(g("id_linha")),"desc_linha":g("desc_linha") or None,
          "desc_marca":g("desc_marca") or None,"desc_colecao":g("desc_colecao") or None,
          "id_classificacao":inteiro(g("id_classificacao")),"desc_classificacao":g("desc_classificacao") or None,
          "desativado":g("desativado")=="S"}
    if novo==0: break
    ts=maxts-1 if maxts>0 else 0
    time.sleep(0.15)
upsert("vendas_produtos", list(produtos.values()), "cod_produto")
print(f"  OK {len(produtos)} produtos sincronizados.")

print("\n[2b] Reconciliando vendas dos ultimos 7 dias...")
LOJAS=get_lojas()
gt=0.0; gi=0
for cod in sorted(CNPJS):
    ...
    while True:
        loops+=1
        if loops>300: break
        body=f'<?xml version="1.0" encoding="utf-8"?><LinxMicrovix><Authentication user="linx_export" password="linx_export"/><ResponseFormat>xml</ResponseFormat><Command><Name>LinxMovimento</Name><Parameters><Parameter id="chave">{CHAVE}</Parameter><Parameter id="cnpjEmp">{cnpj}</Parameter><Parameter id="timestamp">{ts}</Parameter><Parameter id="data_inicial">{VEND_DI}</Parameter><Parameter id="data_fim">{VEND_DF}</Parameter></Parameters></Command></LinxMicrovix>'
        req=urllib.request.Request(LINX,data=body.encode("utf-8"),headers={"Content-Type":"text/xml; charset=utf-8"})
        try:
            with urllib.request.urlopen(req,timeout=300) as r: xml=r.read().decode("utf-8")
        except Exception as e: print(f"  loja {cod} erro {e}, repetindo"); time.sleep(4); continue
        rd=ET.fromstring(xml).find("ResponseData")
        if rd is None or rd.find("C") is None: break
        cols=[(d.text or "") for d in rd.find("C").findall("D")]; ix={c:i for i,c in enumerate(cols)}
        rows=rd.findall("R")
    ...
    upsert("vendas_itens", regs, "transacao")
    sub=round(sum(realizado(x) for x in regs),2)
    gt+=sub; gi+=len(regs)
    print(f"  loja {cod:>2}  {len(regs):>4} itens | R$ {sub:>11,.2f}", flush=True)
print(f"\n  OK {gi} itens reconciliados | realizado 7 dias R$ {gt:,.2f}")

print("\n[cat] Preenchendo categoria dos itens novos...")
def chama():
    ...
    with urllib.request.urlopen(req,timeout=650) as r: return int(r.read().decode().strip())
tot=0
while True:
    n=chama(); tot+=n
    if n==0: break
